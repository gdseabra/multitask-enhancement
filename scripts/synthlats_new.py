...
def add_background_image(img):
    ...
    # return ((img_float * bg_float)/255).astype(np.uint8)
...
def add_random_occlusions(img, max_squares=10, max_lines=3):
    img = img.copy()
    h, w = img.shape[:2]

    # Add random black squares
    for _ in range(random.randint(1, max_squares)):
        square_size = random.randint(5, 20)
...
def parse_args():
    ...
    parser.add_argument("background_path", type=str, help="Folder with background images") # /storage/backgrounds/images/imagesa/a/a
...
def process_image(args):
    try:
        image_path, output_dir, background_path, num_synthetic = args
...
def main(args):
    ...
    task_args = [(image, args.output, args.background_path, args.n) for image in images]
