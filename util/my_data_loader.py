...
class ReferenceDataset(Dataset):
    def __init__(self, reference, input_row, input_col, step):
        self.input_row = input_row
        self.input_col = input_col
        shape_ref = reference.shape
        ROW = shape_ref[0]
        COL = shape_ref[1]
        row_list_1 = range(input_row, ROW+1, step)
        row_list_2 = range(ROW, row_list_1[-1]-1,-step)
        ...
        self.reference = reference.reshape(1,ROW,COL).detach().clone()
        ind = 0
        for row_ind in row_list:
            for col_ind in col_list:
                row_col_inds[ind,:] = [row_ind,col_ind]
    ...
    def __getitem__(self, ind):
        ...
        row = row_col[0]
        col = row_col[1]
        patch = self.reference[:,(row-self.input_row):row,(col-self.input_col):col].detach().clone()
        return row_col, patch
